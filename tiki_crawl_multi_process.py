...
conn = sqlite3.connect('tiki_v2.db')
cur = conn.cursor()
...
def get_sub_cat_by_MP(category, save_db=True):
    name = category[1]
    if category[3] != None:
...
# conn.commit()
# cate = cur.execute('SELECT * FROM categories;').fetchall()


# with concurrent.futures.ProcessPoolExecutor() as executor:
#     results = executor.map(get_sub_cat_by_MP, cate)
conn.close()
